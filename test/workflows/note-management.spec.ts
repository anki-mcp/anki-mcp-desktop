...
import { mockNotes } from '../../src/test-fixtures/mock-data';
...
describe('Note Management Workflow', () => {
  ...
  beforeEach(async () => {
    ...
    updateNoteFieldsTool = module.get<UpdateNoteFieldsTool>(UpdateNoteFieldsTool);
  ...
  describe('Complete Note Lifecycle', () => {
    it('should complete full CRUD workflow for a note', async () => {
      ...
      expect(verifyResult.notes[0].fields.Front.value).toBe(updatedFields.Front);
    ...
    it('should handle search and filter workflow', async () => {
      ...
      const allNoteIds = [3001, 3002, 3003, 3004, 3005];
